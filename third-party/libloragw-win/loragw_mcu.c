...
int spi_req_bulk_insert(spi_req_bulk_t * bulk_buffer, uint8_t * req, uint16_t req_size) {
    ...
    if (bulk_buffer->nb_req == 255) {
        ERROR_PRINTF("ERROR: cannot insert a new SPI request in bulk buffer - too many requests\n");
        return -1;
    }

    if ((bulk_buffer->size + req_size) > LGW_USB_BURST_CHUNK) {
        ERROR_PRINTF("ERROR: cannot insert a new SPI request in bulk buffer - buffer full\n");
        return -1;
    }
...
int write_req(int fd, order_id_t cmd, const uint8_t * payload, uint16_t payload_size ) {
    ...
    if (payload_size > MAX_SIZE_COMMAND) {
        ERROR_PRINTF("ERROR: payload size exceeds maximum transfer size (req:%u, max:%d)\n", payload_size, MAX_SIZE_COMMAND);
        return -1;
    }
    ...
    if (n < 0) {
        ERROR_PRINTF("ERROR: failed to write command header to com port\n");
        return -1;
    }
    ...
    if (payload_size > 0) {
        if (payload == NULL) {
            ERROR_PRINTF("ERROR: invalid payload\n");
            return -1;
        }
        n = write(fd, payload, payload_size);
        if (n < 0) {
            ERROR_PRINTF("ERROR: failed to write command payload to com port\n");
            return -1;
        }
    }
...
int read_ack(int fd, uint8_t * hdr, uint8_t * buf, size_t buf_size) {
...
#if DEBUG_VERBOSE
    DEBUG_PRINTF("read_ack(hdr):");
    /* debug print */
    for (i = 0; i < (int)(HEADER_CMD_SIZE); i++) {
        DEBUG_PRINTF("%02X ", hdr[i]);
    }
    DEBUG_PRINTF("\n");
#endif
    ...
    if ((cmd_get_type(hdr) < 0x40) || (cmd_get_type(hdr) > 0x46)) {
        ERROR_PRINTF("ERROR: received wrong ACK type (0x%02X)\n", cmd_get_type(hdr));
        return -1;
    }
    ...
    if (size > buf_size) {
        ERROR_PRINTF("ERROR: not enough memory to store all data (%zd)\n", size);
        return -1;
    }
...
int decode_ack_ping(const uint8_t * hdr, const uint8_t * payload, s_ping_info * info) {
    ...
    if ((hdr == NULL) || (payload == NULL) || (info == NULL)) {
        ERROR_PRINTF("ERROR: invalid parameter\n");
        return -1;
    }

    if (cmd_get_type(hdr) != ORDER_ID__ACK_PING) {
        ERROR_PRINTF("ERROR: wrong ACK type for PING (expected:0x%02X, got 0x%02X)\n", ORDER_ID__ACK_PING, cmd_get_type(hdr));
        return -1;
    }
...
int decode_ack_bootloader_mode(const uint8_t * hdr) {
    ...
    if (cmd_get_type(hdr) != ORDER_ID__ACK_BOOTLOADER_MODE) {
        ERROR_PRINTF("ERROR: wrong ACK type for ACK_BOOTLOADER_MODE (expected:0x%02X, got 0x%02X)\n", ORDER_ID__ACK_BOOTLOADER_MODE, cmd_get_type(hdr));
        return -1;
    }

#if DEBUG_VERBOSE
    DEBUG_PRINTF("## ACK_BOOTLOADER_MODE\n");
    DEBUG_PRINTF("   id:           0x%02X\n", cmd_get_id(hdr));
    DEBUG_PRINTF("   size:         %u\n", cmd_get_size(hdr));
#endif
...
int decode_ack_get_status(const uint8_t * hdr, const uint8_t * payload, s_status * status) {
    ...
    if (cmd_get_type(hdr) != ORDER_ID__ACK_GET_STATUS) {
        ERROR_PRINTF("ERROR: wrong ACK type for GET_STATUS (expected:0x%02X, got 0x%02X)\n", ORDER_ID__ACK_GET_STATUS, cmd_get_type(hdr));
        return -1;
    }
...
int decode_ack_gpio_access(const uint8_t * hdr, const uint8_t * payload, uint8_t * write_status) {
    ...
    if (cmd_get_type(hdr) != ORDER_ID__ACK_WRITE_GPIO) {
        ERROR_PRINTF("ERROR: wrong ACK type for WRITE_GPIO (expected:0x%02X, got 0x%02X)\n", ORDER_ID__ACK_WRITE_GPIO, cmd_get_type(hdr));
        return -1;
    }
...
int decode_ack_spi_access(const uint8_t * hdr, const uint8_t * payload) {
    ...
    if (cmd_get_type(hdr) != ORDER_ID__ACK_SPI) {
        ERROR_PRINTF("ERROR: wrong ACK type for ACK_SPI (expected:0x%02X, got 0x%02X)\n", ORDER_ID__ACK_SPI, cmd_get_type(hdr));
        return -1;
    }

#if DEBUG_VERBOSE
    DEBUG_PRINTF("## ACK_SPI_ACCESS\n");
    DEBUG_PRINTF("   id:           0x%02X\n", cmd_get_id(hdr));
    DEBUG_PRINTF("   size:         %u\n", cmd_get_size(hdr));
#endif
...
int decode_ack_spi_bulk(const uint8_t * hdr, const uint8_t * payload) {
    ...
    if (cmd_get_type(hdr) != ORDER_ID__ACK_MULTIPLE_SPI) {
        ERROR_PRINTF("ERROR: wrong ACK type for ACK_MULTIPLE_SPI (expected:0x%02X, got 0x%02X)\n", ORDER_ID__ACK_MULTIPLE_SPI, cmd_get_type(hdr));
        return -1;
    }

#if DEBUG_VERBOSE
    DEBUG_PRINTF("## ACK_SPI_BULK\n");
    DEBUG_PRINTF("   id:           0x%02X\n", cmd_get_id(hdr));
    DEBUG_PRINTF("   size:         %u\n", cmd_get_size(hdr));
#endif
...
int mcu_ping(int fd, s_ping_info * info) {
    ...
    if (write_req(fd, ORDER_ID__REQ_PING, NULL, 0) != 0) {
        ERROR_PRINTF("ERROR: failed to write PING request\n");
        return -1;
    }

    if (read_ack(fd, buf_hdr, buf_ack, sizeof buf_ack) < 0) {
        ERROR_PRINTF("ERROR: failed to read PING ack\n");
        return -1;
    }

    if (decode_ack_ping(buf_hdr, buf_ack, info) != 0) {
        ERROR_PRINTF("ERROR: invalid PING ack\n");
        return -1;
    }
...
int mcu_boot(int fd) {
    if (write_req(fd, ORDER_ID__REQ_BOOTLOADER_MODE, NULL, 0) != 0) {
        ERROR_PRINTF("ERROR: failed to write BOOTLOADER_MODE request\n");
        return -1;
    }

    if (read_ack(fd, buf_hdr, NULL, 0) < 0) {
        ERROR_PRINTF("ERROR: failed to read BOOTLOADER_MODE ack\n");
        return -1;
    }

    if (decode_ack_bootloader_mode(buf_hdr) != 0) {
        ERROR_PRINTF("ERROR: invalid BOOTLOADER_MODE ack\n");
        return -1;
    }
...
int mcu_get_status(int fd, s_status * status) {
    ...
    if (write_req(fd, ORDER_ID__REQ_GET_STATUS, NULL, 0) != 0) {
        ERROR_PRINTF("ERROR: failed to write GET_STATUS request\n");
        return -1;
    }

    if (read_ack(fd, buf_hdr, buf_ack, sizeof buf_ack) < 0) {
        ERROR_PRINTF("ERROR: failed to read GET_STATUS ack\n");
        return -1;
    }

    if (decode_ack_get_status(buf_hdr, buf_ack, status) != 0) {
        ERROR_PRINTF("ERROR: invalid GET_STATUS ack\n");
        return -1;
    }
...
int mcu_gpio_write(int fd, uint8_t gpio_port, uint8_t gpio_id, uint8_t gpio_value) {
    ...
    if (write_req(fd, ORDER_ID__REQ_WRITE_GPIO, buf_req, REQ_WRITE_GPIO_SIZE) != 0) {
        ERROR_PRINTF("ERROR: failed to write REQ_WRITE_GPIO request\n");
        return -1;
    }
    ...
    if (decode_ack_gpio_access(buf_hdr, buf_ack, &status) != 0) {
        ERROR_PRINTF("ERROR: invalid REQ_WRITE_GPIO ack\n");
        return -1;
    }

    if (status != 0) {
        ERROR_PRINTF("ERROR: Failed to write GPIO (port:%u id:%u value:%u)\n", gpio_port, gpio_id, gpio_value);
        return -1;
    }
...
int mcu_spi_write(int fd, uint8_t * in_out_buf, size_t buf_size) {
    ...
    if (write_req(fd, ORDER_ID__REQ_MULTIPLE_SPI, in_out_buf, buf_size) != 0) {
        ERROR_PRINTF("ERROR: failed to write REQ_MULTIPLE_SPI request\n");
        return -1;
    }

    if (read_ack(fd, buf_hdr, in_out_buf, buf_size) < 0) {
        ERROR_PRINTF("ERROR: failed to read REQ_MULTIPLE_SPI ack\n");
        return -1;
    }

    if (decode_ack_spi_bulk(buf_hdr, in_out_buf) != 0) {
        ERROR_PRINTF("ERROR: invalid REQ_MULTIPLE_SPI ack\n");
        return -1;
    }
...
int mcu_spi_flush(int fd) {
    ...
    if (mcu_spi_write(fd, spi_bulk_buffer.buffer, spi_bulk_buffer.size) != 0) {
        ERROR_PRINTF("ERROR: %s: failed to write SPI requests to MCU\n", __FUNCTION__);
        return -1;
    }
