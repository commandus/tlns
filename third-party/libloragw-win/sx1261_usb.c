...
int sx1261_usb_w(void *com_target, sx1261_op_code_t op_code, uint8_t *data, uint16_t size) {
    ...
    if (a != 0) {
        ERROR_PRINTF("ERROR: USB SX1261 WRITE FAILURE\n");
        return -1;
    } else {
        DEBUG_PRINTF("Note: USB SX1261 write success\n");
        return 0;
    }
}
...
int sx1261_usb_set_write_mode(lgw_com_write_mode_t write_mode) {
    if (write_mode >= LGW_COM_WRITE_MODE_UNKNOWN) {
        ERROR_PRINTF("ERROR: %s: wrong write mode\n", __FUNCTION__);
        return -1;
    }

    INFO_PRINTF("setting SX1261 USB write mode to %s\n", (write_mode == LGW_COM_WRITE_MODE_SINGLE) ? "SINGLE" : "BULK");
...
int sx1261_usb_flush(void *com_target) {
    ...
    if (_sx1261_spi_req_nb == 0) {
        INFO_PRINTF("no SX1261 SPI request to flush\n");
        return 0;
    }

    usb_device = *(int *)com_target;

    INFO_PRINTF("flushing SX1261 USB write buffer\n");
    a = mcu_spi_flush(usb_device);
    if (a != 0) {
        ERROR_PRINTF("ERROR: Failed to flush sx1261 USB write buffer\n");
    }
