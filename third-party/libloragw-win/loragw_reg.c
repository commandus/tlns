...
int lgw_connect(const lgw_com_type_t com_type, const char * com_path) {
    ...
    if (com_path == NULL) {
        ERROR_PRINTF("ERROR: COM PATH IS NOT SET\n");
        return LGW_REG_ERROR;
    }
...
int lgw_reg_w(uint16_t register_id, int32_t reg_value) {
    ...
    if (r.rdon == 1){
        ERROR_PRINTF("ERROR: TRYING TO WRITE A READ-ONLY REGISTER\n");
        return LGW_REG_ERROR;
    }
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING REGISTER WRITE\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int lgw_reg_wb(uint16_t register_id, uint8_t *data, uint16_t size) {
    ...
    /* reject write to read-only registers */
    if (r.rdon == 1){
        ERROR_PRINTF("ERROR: TRYING TO BURST WRITE A READ-ONLY REGISTER\n");
        return LGW_REG_ERROR;
    }
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING REGISTER BURST WRITE\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int lgw_reg_rb(uint16_t register_id, uint8_t *data, uint16_t size) {
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING REGISTER BURST READ\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
