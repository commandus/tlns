...
int calculate_freq_to_time_drift(uint32_t freq_hz, uint8_t bw, uint16_t * mant, uint8_t * exp) {
    ...
    if (bw_hz < 0) {
        ERROR_PRINTF("ERROR: Unsupported bandwidth for frequency to time drift calculation\n");
...
int sx1302_init(const struct lgw_conf_ftime_s * ftime_context) {
    ...
    if (ftime_context->enable == true) {
        ...
        if (x != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to getUplink Chip Model ID\n");
            return LGW_REG_ERROR;
        }

        if (model_id != CHIP_MODEL_ID_SX1303) {
            ERROR_PRINTF("ERROR: Fine Timestamping is not supported on this Chip Model ID 0x%02X\n", model_id);
            return LGW_REG_ERROR;
        }
    }
    x = timestamp_counter_mode(ftime_context->enable);
    if (x != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure timestamp counter mode\n");
        return LGW_REG_ERROR;
    }

    x = sx1302_config_gpio();
    if (x != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure sx1302 GPIOs\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_update(void) {
    ...
    if (val != 0) {
        ERROR_PRINTF("ERROR: Parity error check failed on AGC firmware\n");
        return LGW_REG_ERROR;
    }
    lgw_reg_r(SX1302_REG_ARB_MCU_CTRL_PARITY_ERROR, &val);
    if (val != 0) {
        ERROR_PRINTF("ERROR: Parity error check failed on ARB firmware\n");
        return LGW_REG_ERROR;
    }
#endif
...
int sx1302_radio_clock_select(uint8_t rf_chain) {
    ...
    switch (rf_chain) {
        case 0:
            DEBUG_PRINTF("Select Radio A clock\n");
            ...
            break;
        case 1:
            DEBUG_PRINTF("Select Radio B clock\n");
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to select radio clock for radio_%u\n", rf_chain);
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_reset(uint8_t rf_chain, lgw_radio_type_t type) {
    ...
    switch (type) {
        ...
        case LGW_RADIO_TYPE_SX1257:
            /* Do nothing */
           INFO_PRINTF("reset sx125x (RADIO_%s) done\n", REG_SELECT(rf_chain, "A", "B"));
            break;
        case LGW_RADIO_TYPE_SX1250:
            err |= lgw_reg_w(reg_radio_rst, 0x01);
            wait_ms(10); /* wait for auto calibration to complete */
           INFO_PRINTF("reset sx1250 (RADIO_%s) done\n", REG_SELECT(rf_chain, "A", "B"));
            break;
        default:
            return LGW_REG_ERROR;
    }
    ...
    /* Check if something went wrong */
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to reset the radios\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_set_mode(uint8_t rf_chain, lgw_radio_type_t type) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to set mode for radio %u\n", rf_chain);
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_calibrate(struct lgw_conf_rxrf_s * context_rf_chain, uint8_t clksrc, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        if (context_rf_chain[i].enable == true) {
            ...
            if (err != LGW_REG_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to reset radio %d\n", i);
                return LGW_REG_ERROR;
            }

            err = sx1302_radio_set_mode(i, context_rf_chain[i].type);
            if (err != LGW_REG_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to set radio %d mode\n", i);
                return LGW_REG_ERROR;
            }
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to getUplink select clock from radio %u\n", clksrc);
        return LGW_REG_ERROR;
    }
    ...
    if ((context_rf_chain[clksrc].type == LGW_RADIO_TYPE_SX1257) ||
        ...
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: Failed to load calibration fw\n");
            return LGW_REG_ERROR;
        }
        err = sx1302_cal_start(FW_VERSION_CAL, context_rf_chain, txgain_lut);
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: radio calibration failed\n");
...
int sx1302_lora_service_correlator_configure(struct lgw_conf_rxif_s * cfg) {
    ...
    switch (cfg->datarate) {
        ...
        case 12:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: Failed to configure LoRa service modem correlators\n");
            return LGW_REG_ERROR;
    }
...
int sx1302_lora_modem_configure(uint32_t radio_freq_hz) {
    ...
    /* Enable full modems */
    DEBUG_PRINTF("Configuring 8 full-SF modems\n");
    err |= lgw_reg_w(SX1302_REG_OTP_MODEM_EN_0_MODEM_EN, 0xFF);

    /* Enable limited modems */
    DEBUG_PRINTF("Configuring 8 limited-SF modems\n");
    ...
    if (calculate_freq_to_time_drift(radio_freq_hz, BW_125KHZ, &mantissa, &exponent) != 0) {
        ERROR_PRINTF("ERROR: failed to calculate frequency to time drift for LoRa modem\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_lora_service_modem_configure(struct lgw_conf_rxif_s * cfg, uint32_t radio_freq_hz) {
        ...
        case DR_LORA_SF12:
            ...
            switch (cfg->bandwidth) {
                ...
                case BW_500KHZ:
                    ...
                    break;
                default:
                    ERROR_PRINTF("ERROR: unsupported bandwidth %u for LoRa Service modem\n", cfg->bandwidth);
                    break;
            }
            break;
        default:
            ERROR_PRINTF("ERROR: unsupported datarate %u for LoRa Service modem\n", cfg->datarate);
            break;
    }
    ...
    INFO_PRINTF("LoRa Service modem: configuring preamble size to %u symbols\n", preamble_nb_symb);
    ...
    if (calculate_freq_to_time_drift(radio_freq_hz, cfg->bandwidth, &mantissa, &exponent) != 0) {
        ERROR_PRINTF("ERROR: failed to calculate frequency to time drift for LoRa service modem\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_lora_syncword(bool public, uint8_t lora_service_sf) {
    ...
    INFO_PRINTF("configuring LoRa (Multi-SF) SF5->SF6 with syncword PRIVATE (0x12)\n");
    ...
    if (public == true) {
        INFO_PRINTF("configuring LoRa (Multi-SF) SF7->SF12 with syncword PUBLIC (0x34)\n");
        ...
        INFO_PRINTF("configuring LoRa (Multi-SF) SF7->SF12 with syncword PRIVATE (0x12)\n");
    ...
    if ((public == false) || (lora_service_sf == DR_LORA_SF5) || (lora_service_sf == DR_LORA_SF6)) {
        INFO_PRINTF("configuring LoRa (Service) SF%u with syncword PRIVATE (0x12)\n", lora_service_sf);
        ...
        INFO_PRINTF("configuring LoRa (Service) SF%u with syncword PUBLIC (0x34)\n", lora_service_sf);
...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    ...
    if (memcmp(firmware, fw_check, sizeof fw_check) != 0) {
        ERROR_PRINTF("ERROR: AGC fw read/write check failed\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != 0) {
        ERROR_PRINTF("ERROR: Failed to load AGC fw: parity error check failed\n");
        return LGW_REG_ERROR;
    }
    DEBUG_PRINTF("AGC fw loaded\n");
...
int sx1302_agc_status(uint8_t* status) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: Failed to getUplink AGC status\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_agc_mailbox_write(uint8_t mailbox, uint8_t value) {
    ...
    if (lgw_reg_w(reg, (int32_t)value) != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to write AGC mailbox\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_agc_start(uint8_t version, lgw_radio_type_t radio_type, uint8_t ana_gain, uint8_t dec_gain, bool full_duplex, bool lbt_enable) {
    ...
    if (val != version) {
        ERROR_PRINTF("ERROR: wrong AGC fw version (%d)\n", val);
        return LGW_REG_ERROR;
    }
    ...
    if (radio_type != LGW_RADIO_TYPE_SX1250) {
        INFO_PRINTF("AGC: setting fdd_mode to %u\n", fdd_mode);
    ...
    sx1302_agc_mailbox_read(0, &val);
    if (val != ana_gain) {
        ERROR_PRINTF("ERROR: Analog gain of Radio A has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != dec_gain) {
        ERROR_PRINTF("ERROR: Decimator gain of Radio A has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != fdd_mode) {
        ERROR_PRINTF("ERROR: FDD mode of Radio A has not been set properly\n");
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: Radio A getConfigForCOMPortAndRegion done\n");
    ...
    sx1302_agc_mailbox_read(0, &val);
    if (val != ana_gain) {
        ERROR_PRINTF("ERROR: Analog gain of Radio B has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    sx1302_agc_mailbox_read(1, &val);
    if (val != dec_gain) {
        ERROR_PRINTF("ERROR: Decimator gain of Radio B has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    sx1302_agc_mailbox_read(2, &val);
    if (val != fdd_mode) {
        ERROR_PRINTF("ERROR: FDD mode of Radio B has not been set properly\n");
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: Radio B getConfigForCOMPortAndRegion done\n");
    ...
    if (val != agc_params.ana_min) {
        ERROR_PRINTF("ERROR: wrong ana_min (w:%u r:%u)\n", agc_params.ana_min, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.ana_max) {
        ERROR_PRINTF("ERROR: ana_max (w:%u r:%u)\n", agc_params.ana_max, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of analog gain min/max done\n");
    ...
    if (val != agc_params.ana_thresh_l) {
        ERROR_PRINTF("ERROR: wrong ana_thresh_l (w:%u r:%u)\n", agc_params.ana_thresh_l, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.ana_thresh_h) {
        ERROR_PRINTF("ERROR: wrong ana_thresh_h (w:%u r:%u)\n", agc_params.ana_thresh_h, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of analog threshold done\n");
    ...
    if (val != agc_params.dec_attn_min) {
        ERROR_PRINTF("ERROR: wrong dec_attn_min (w:%u r:%u)\n", agc_params.dec_attn_min, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.dec_attn_max) {
        ERROR_PRINTF("ERROR: wrong dec_attn_max (w:%u r:%u)\n", agc_params.dec_attn_max, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of decimator atten min/max done\n");
    ...
    if (val != agc_params.dec_thresh_l) {
        ERROR_PRINTF("ERROR: wrong dec_thresh_l (w:%u r:%u)\n", agc_params.dec_thresh_l, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.dec_thresh_h1) {
        ERROR_PRINTF("ERROR: wrong dec_thresh_h1 (w:%u r:%u)\n", agc_params.dec_thresh_h1, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(2, &val);
    if (val != agc_params.dec_thresh_h2) {
        ERROR_PRINTF("ERROR: wrong dec_thresh_h2 (w:%u r:%u)\n", agc_params.dec_thresh_h2, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of decimator threshold done\n");
    ...
    if (val != agc_params.chan_attn_min) {
        ERROR_PRINTF("ERROR: wrong chan_attn_min (w:%u r:%u)\n", agc_params.chan_attn_min, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.chan_attn_max) {
        ERROR_PRINTF("ERROR: wrong chan_attn_max (w:%u r:%u)\n", agc_params.chan_attn_max, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of channel atten min/max done\n");
    ...
    if (val != agc_params.chan_thresh_l) {
        ERROR_PRINTF("ERROR: wrong chan_thresh_l (w:%u r:%u)\n", agc_params.chan_thresh_l, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.chan_thresh_h) {
        ERROR_PRINTF("ERROR: wrong chan_thresh_h (w:%u r:%u)\n", agc_params.chan_thresh_h, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of channel atten threshold done\n");
    ...
    if (radio_type == LGW_RADIO_TYPE_SX1250) {
        ...
        if (val != agc_params.deviceSel) {
            ERROR_PRINTF("ERROR: wrong deviceSel (w:%u r:%u)\n", agc_params.deviceSel, val);
            return LGW_REG_ERROR;
        }
        sx1302_agc_mailbox_read(1, &val);
        if (val != agc_params.hpMax) {
            ERROR_PRINTF("ERROR: wrong hpMax (w:%u r:%u)\n", agc_params.hpMax, val);
            return LGW_REG_ERROR;
        }
        sx1302_agc_mailbox_read(2, &val);
        if (val != agc_params.paDutyCycle) {
            ERROR_PRINTF("ERROR: wrong paDutyCycle (w:%u r:%u)\n", agc_params.paDutyCycle, val);
            return LGW_REG_ERROR;
        }

        DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of sx1250 PA optimal settings done\n");
    }
    ...
    if (val != pa_start_delay) {
        ERROR_PRINTF("ERROR: wrong PA start delay (w:%u r:%u)\n", pa_start_delay, val);
        return LGW_REG_ERROR;
    }

    DEBUG_PRINTF("AGC: getConfigForCOMPortAndRegion of PA start delay done\n");
    ...
    if ((bool)val != lbt_enable) {
        ERROR_PRINTF("ERROR: wrong LBT configuration (w:%u r:%u)\n", lbt_enable, val);
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_load_firmware(const uint8_t *firmware) {
    ...
    if (memcmp(firmware, fw_check, sizeof fw_check) != 0) {
        ERROR_PRINTF("ERROR: ARB fw read/write check failed\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != 0) {
        ERROR_PRINTF("ERROR: Failed to load ARB fw: parity error check failed\n");
        return LGW_REG_ERROR;
    }
    DEBUG_PRINTF("ARB fw loaded\n");
...
int sx1302_arb_status(uint8_t* status) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: Failed to getUplink ARB status\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_debug_read(uint8_t reg_id, uint8_t* value) {
    ...
    /* Check parameters */
    if (reg_id > 15) {
        ERROR_PRINTF("ERROR: invalid ARB debug register ID\n");
        return LGW_REG_ERROR;
    }

    reg = SX1302_REG_ARB_MCU_ARB_DEBUG_STS_0_ARB_DEBUG_STS_0 + reg_id;
    if (lgw_reg_r(reg, &val) != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to read ARB debug register\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_debug_write(uint8_t reg_id, uint8_t value) {
    ...
    if (lgw_reg_w(reg, (int32_t)value) != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to write ARB debug register ID\n");
        return LGW_REG_ERROR;
    }
...
void sx1302_arb_set_debug_stats(bool enable, uint8_t sf) {
    if (enable == true) {
        ...
        DEBUG_PRINTF("ARB: Debug stats disabled\n");
    }
}
...
uint8_t sx1302_arb_get_debug_stats_detect(uint8_t channel) {
    ...
    if (channel >= 8) {
        ERROR_PRINTF("ERROR: wrong configuration, channel num must be < 8");
        return 0;
    }
...
int sx1302_arb_start(uint8_t version, const struct lgw_conf_ftime_s * ftime_context) {
    ...
    if (val != version) {
        ERROR_PRINTF("ERROR: wrong ARB fw version (%d)\n", val);
        return LGW_REG_ERROR;
    }
...
int sx1302_fetch(uint8_t * nb_pkt) {
    ...
    if (rx_buffer.buffer_pkt_nb == 0) {
        ...
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: Failed to initialize RX buffer\n");
            return LGW_REG_ERROR;
        }
        ...
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: Failed to fetch RX buffer\n");
            return LGW_REG_ERROR;
        }
    } else {
        INFO_PRINTF("Note: remaining %u packets in RX buffer, do not fetch sx1302 yet...\n", rx_buffer.buffer_pkt_nb);
    }
    ...
    */
    ...
    if ((ifmod == IF_LORA_MULTI) || (ifmod == IF_LORA_STD)) {
            ...
            if (pkt.payload_crc_error) {
                ...
                if (p->size > 0) {
                    ...
                    if (payload_crc16_calc != pkt.rx_crc16_value) {
                        ERROR_PRINTF("ERROR: Payload CRC16 check failed (got:0x%04X calc:0x%04X)\n", pkt.rx_crc16_value, payload_crc16_calc);
                        return LGW_REG_ERROR;
                    } else {
        ...
        if (p->status == STAT_CRC_OK || p->status == STAT_NO_CRC) {
            ...
            */
            ...
            for (i = 0; i < context->debug_cfg.nb_ref_payload; i++) {
                ...
                if (res == -1) {
                    ERROR_PRINTF("ERROR: 0x%08X payload error\n", context->debug_cfg.ref_payload[i].id);
                    return LGW_REG_ERROR;
                } else if (res == 1) {
        ...
        switch (p->bandwidth) {
            ...
            default:
                p->freq_offset = 0;
                INFO_PRINTF("Invalid frequency offset\n");
                break;
        }
        ...
        if (pkt.crc_en) {
            /* CRC enabled */
            if (pkt.payload_crc_error) {
                INFO_PRINTF("FSK: CRC ERR\n");
                p->status = STAT_CRC_BAD;
            } else {
                INFO_PRINTF("FSK: CRC OK\n");
                p->status = STAT_CRC_OK;
            }
        } else {
            /* CRC disabled */
        ...
        ERROR_PRINTF("ERROR: UNEXPECTED PACKET ORIGIN\n");
...
uint16_t sx1302_lora_payload_crc(const uint8_t * data, uint8_t size) {
    ...
    //INFO_PRINTF("CRC16: 0x%02X 0x%02X (%X)\n", (uint8_t)(crc >> 8), (uint8_t)crc, crc);
    return (uint16_t)crc;
}
...
float sx1302_rssi_get_temperature_offset(struct lgw_rssi_tcomp_s * context, float temperature) {
    ...
    DEBUG_PRINTF("RSSI temperature compensation:\n");
...
uint8_t sx1302_tx_status(uint8_t rf_chain) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: Failed to read TX STATUS\n");
    ...
    if (read_value == 0x80) {
        ...
        ERROR_PRINTF("ERROR: UNKNOWN TX STATUS 0x%02X\n", read_value);
        return TX_STATUS_UNKNOWN;
    }
}
...
int sx1302_tx_abort(uint8_t rf_chain) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: Failed to stop TX trigger\n");
        return err;
    }
    ...
    do {
        ...
        if (timeout_check(tm_start, 1000) != 0) {
            ERROR_PRINTF("ERROR: %s: TIMEOUT on TX abort\n", __FUNCTION__);
            return LGW_REG_ERROR;
        }
...
int sx1302_send(lgw_radio_type_t radio_type, struct lgw_tx_gain_lut_s * tx_lut, bool lwan_public, struct lgw_conf_rxif_s * context_fsk, struct lgw_pkt_tx_s * pkt_data) {
    ...
    switch (pkt_data->modulation) {
        ...
        case MOD_FSK:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: modulation type not supported\n");
            return LGW_REG_ERROR;
    }
    ...
    INFO_PRINTF("selecting TX Gain LUT index %u\n", pow_index);
    ...
    switch (pkt_data->modulation) {
        ...
        case MOD_FSK:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: Modulation not supported\n");
            return LGW_REG_ERROR;
    }
    ...
    switch (pkt_data->modulation) {
        ...
        case MOD_LORA:
            ...
            if (pkt_data->preamble == 0) { /* if not explicit, use recommended LoRa preamble size */
                ...
                DEBUG_PRINTF("Note: preamble length adjusted to respect minimum LoRa preamble size\n");
            }
        ...
        case MOD_FSK:
            ...
            if (pkt_data->preamble == 0) { /* if not explicit, use LoRaWAN preamble size */
                ...
                DEBUG_PRINTF("Note: preamble length adjusted to respect minimum FSK preamble size\n");
            }
    ...
    switch (pkt_data->tx_mode) {
        ...
        case ON_GPS:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: TX mode not supported\n");
            return LGW_REG_ERROR;
    }
