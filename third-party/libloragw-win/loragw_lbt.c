...
int lgw_lbt_start(const struct lgw_conf_sx1261_s * sx1261_context, const struct lgw_pkt_tx_s * pkt) {
    ...
    if (lbt_channel_selected == -1) {
        ERROR_PRINTF("ERROR: Cannot start LBT - wrong channel\n");
        return -1;
    }
    ...
    if (sx1261_context->lbt_conf.channels[lbt_channel_selected].transmit_time_ms * 1000 <= 1500) {
        ERROR_PRINTF("ERROR: Cannot start LBT - channel transmit_time_ms must be > 1.5ms\n");
        return -1;
    }
    ...
    if ((toa_ms * 1000) > (uint32_t)(sx1261_context->lbt_conf.channels[lbt_channel_selected].transmit_time_ms * 1000 - 1500)) {
        ERROR_PRINTF("ERROR: Cannot start LBT - packet time on air exceeds allowed transmit time (toa:%ums, max:%ums)\n", toa_ms, sx1261_context->lbt_conf.channels[lbt_channel_selected].transmit_time_ms);
        return -1;
    }
    ...
    if (err != 0) {
        ERROR_PRINTF("ERROR: Cannot start LBT - unable to set sx1261 RX parameters\n");
        return -1;
    }
    ...
    if (err != 0) {
        ERROR_PRINTF("ERROR: Cannot start LBT - sx1261 LBT start\n");
        return -1;
    }
...
int lgw_lbt_tx_status(uint8_t rf_chain, bool * tx_ok) {
    ...
    do {
        ...
        if (timeout_check(tm_start, 500) != 0) {
            ERROR_PRINTF("ERROR: %s: TIMEOUT on TX start, not started\n", __FUNCTION__);
            tx_timeout = true;
        ...
        if (err != 0) {
            ERROR_PRINTF("ERROR: %s: failed to getUplink AGC status\n", __FUNCTION__);
            return -1;
        }
        wait_ms(1);
    ...
    do {
        /* handle timeout */
        if (timeout_check(tm_start, 500) != 0) {
            ERROR_PRINTF("ERROR: %s: TIMEOUT on TX start (AGC clear status)\n", __FUNCTION__);
            tx_timeout = true;
            break;
        }
...
int lgw_lbt_stop(void) {
    ...
    err = sx1261_lbt_stop();
    if (err != 0) {
        ERROR_PRINTF("ERROR: Cannot stop LBT - failed\n");
        return -1;
    }
