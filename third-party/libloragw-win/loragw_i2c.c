...
#if DEBUG_I2C == 1
    #define DEBUG_PRINTF(str)                INFO_PRINTF(str)
    #define DEBUG_PRINTF(fmt, args...)    INFO_PRINTF("%s:%d: "fmt, __FUNCTION__, __LINE__, args)
    #define CHECK_NULL(a)                if(a==NULL){ERROR_PRINTF("%s:%d: ERROR: NULL POINTER AS ARGUMENT\n", __FUNCTION__, __LINE__);return LGW_I2C_ERROR;}
#else
...
int i2c_linuxdev_open(const char *path, uint8_t device_addr, int *i2c_fd) {
    ...
    if (path == NULL) {
        ERROR_PRINTF("ERROR: null pointer path\n");
        return LGW_I2C_ERROR;
    }
    if (i2c_fd == NULL) {
        ERROR_PRINTF("ERROR: null pointer i2c_fd\n");
        return LGW_I2C_ERROR;
    }
    ...
    if (dev < 0) {
        ERROR_PRINTF("ERROR: Failed to open I2C %s - %s\n", path, strerror(errno));
        return LGW_I2C_ERROR;
    }

    /* Setting I2C device mode to slave */
    if (ioctl(dev, I2C_SLAVE, device_addr) < 0) {
        ERROR_PRINTF("ERROR: Failed to acquire bus access and/or talk to slave - %s\n", strerror(errno));
        return LGW_I2C_ERROR;
    }

    INFO_PRINTF("I2C port opened successfully (%s, 0x%02X)\n", path, device_addr);
...
int i2c_linuxdev_read(int i2c_fd, uint8_t device_addr, uint8_t reg_addr, uint8_t *data) {
    ...
    if (ioctl(i2c_fd, I2C_RDWR, &packets) < 0) {
        ERROR_PRINTF("ERROR: Read from I2C Device failed (%d, 0x%02x, 0x%02x) - %s\n", i2c_fd, device_addr, reg_addr, strerror(errno));
        return LGW_I2C_ERROR;
    }
...
int i2c_linuxdev_write(int i2c_fd, uint8_t device_addr, uint8_t reg_addr, uint8_t data) {
    ...
    if (ioctl(i2c_fd, I2C_RDWR, &packets) < 0) {
        ERROR_PRINTF("ERROR: Write to I2C Device failed (%d, 0x%02x, 0x%02x) - %s\n", i2c_fd, device_addr, reg_addr, strerror(errno));
        return LGW_I2C_ERROR;
    }
...
int i2c_linuxdev_write_buffer(int i2c_fd, uint8_t device_addr, uint8_t *buffer, uint8_t size) {
    ...
    if (ioctl(i2c_fd, I2C_RDWR, &packets) < 0) {
        ERROR_PRINTF("ERROR: Write buffer to I2C Device failed (%d, 0x%02x) - %s\n", i2c_fd, device_addr, strerror(errno));
        return LGW_I2C_ERROR;
    }
...
int i2c_linuxdev_close(int i2c_fd) {
    ...
    i = close(i2c_fd);
    if (i == 0) {
        INFO_PRINTF("I2C port closed successfully\n");
        return LGW_I2C_SUCCESS;
    } else {
        ERROR_PRINTF("ERROR: Failed to close I2C - %s\n", strerror(errno));
        return LGW_I2C_ERROR;
    }
}
