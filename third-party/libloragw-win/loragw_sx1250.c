...
int sx1250_reg_w(sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain) {
    ...
    if (rf_chain >= LGW_RF_CHAIN_NB) {
        ERROR_PRINTF("ERROR: INVALID RF_CHAIN\n");
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING RADIO REGISTER WRITE\n");
        return LGW_REG_ERROR;
    } else {
...
int sx1250_reg_r(sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain) {
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING RADIO REGISTER READ\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int sx1250_calibrate(uint8_t rf_chain, uint32_t freq_hz) {
    ...
    if ((freq_hz > 430E6) && (freq_hz < 440E6)) {
        ...
        ERROR_PRINTF("ERROR: failed to calibrate sx1250 radio, frequency range not supported (%u)\n", freq_hz);
        return LGW_REG_ERROR;
    }
    ...
    if (TAKE_N_BITS_FROM(buff[2], 4, 1) != 0) {
        ERROR_PRINTF("ERROR: sx1250 Image Calibration Error\n");
        return LGW_REG_ERROR;
    }
...
int sx1250_setup(uint8_t rf_chain, uint32_t freq_hz, bool single_input_mode) {
    ...
    if ((uint8_t)(TAKE_N_BITS_FROM(buff[0], 4, 3)) != 0x02) {
        ERROR_PRINTF("ERROR: Failed to set SX1250_%u in STANDBY_RC mode\n", rf_chain);
        return LGW_REG_ERROR;
    }
    ...
    if ((uint8_t)(TAKE_N_BITS_FROM(buff[0], 4, 3)) != 0x03) {
        ERROR_PRINTF("ERROR: Failed to set SX1250_%u in STANDBY_XOSC mode\n", rf_chain);
        return LGW_REG_ERROR;
    }
    ...
    if (single_input_mode == true) {
        INFO_PRINTF("Configuring SX1250_%u in single input mode\n", rf_chain);
        buff[0] = 0x08;
        buff[1] = 0xE2;
        buff[2] = 0x0D;
        err |= sx1250_reg_w(WRITE_REGISTER, buff, 3, rf_chain);
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to getConfigForCOMPortAndRegion SX1250_%u radio\n", rf_chain);
        return LGW_REG_ERROR;
    }
