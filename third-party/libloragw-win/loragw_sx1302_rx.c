...
int rx_buffer_fetch(rx_buffer_t * self) {
    ...
    if (self->buffer_size > 0) {
        ...
        if (res != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: Failed to read RX buffer, SPI error\n");
        ...
        if (self->buffer_size < (SX1302_PKT_HEAD_METADATA + SX1302_PKT_TAIL_METADATA)) {
            INFO_PRINTF("WARNING: not enough data to have a complete packet, discard rx_buffer\n");
        ...
        while (idx <= (self->buffer_size - 2)) {
            if ((self->buffer[idx] == SX1302_PKT_SYNCWORD_BYTE_0) && (self->buffer[idx + 1] == SX1302_PKT_SYNCWORD_BYTE_1)) {
               INFO_PRINTF("syncword found at idx %d\n", idx);
                break;
            } else {
                INFO_PRINTF("syncword not found at idx %d\n", idx);
                idx += 1;
            }
        }
        if (idx > self->buffer_size - 2) {
            INFO_PRINTF("WARNING: no syncword found, discard rx_buffer\n");
            return rx_buffer_del(self);
        }
        if (idx != 0) {
            INFO_PRINTF("re-sync rx_buffer at idx %d\n", idx);
        ...
        while (idx < self->buffer_size) {
            if ((self->buffer[idx] != SX1302_PKT_SYNCWORD_BYTE_0) || (self->buffer[idx + 1] != SX1302_PKT_SYNCWORD_BYTE_1)) {
                INFO_PRINTF("WARNING: syncword not found at idx %d, discard the rx_buffer\n", idx);
                return rx_buffer_del(self);
            }
...
int rx_buffer_pop(rx_buffer_t * self, rx_packet_t * pkt) {
    ...
    if (self->buffer_index >= self->buffer_size) {
        INFO_PRINTF("No more data to be parsed\n");
        return LGW_REG_ERROR;
    }
   ...
   INFO_PRINTF("pkt syncword found at index %u\n", self->buffer_index);
    ...
    if((self->buffer_index + pkt_num_bytes) > self->buffer_size) {
        INFO_PRINTF("WARNING: aborting truncated message (size=%u)\n", self->buffer_size);
    ...
    if (checksum_rcv != checksum_calc) {
        INFO_PRINTF("WARNING: checksum failed (got:0x%02X calc:0x%02X)\n", checksum_rcv, checksum_calc);
        return LGW_REG_WARNING;
    } else {
...
void rx_buffer_dump(FILE * file, uint16_t start_addr, uint16_t end_addr) {
    ...
    INFO_PRINTF("Dumping %u bytes, from 0x%X to 0x%X\n", end_addr - start_addr + 1, start_addr, end_addr);
    ...
    for (i = 0; i < (end_addr - start_addr + 1); i++) {
        INFO_PRINTF("%02X ", rx_buffer_debug[i]);
    }
    INFO_PRINTF("\n");
