...
std::string FHDR2String(const FHDR &value, bool downLink = false);
...
DEVNONCE string2DEVNONCE(const char *value);
...
std::string rfm_header2json(const RFM_HEADER* value);
ACTIVATION string2activation(const char *value);
...
LORAWAN_VERSION string2LORAWAN_VERSION(const char *value);
LORAWAN_VERSION string2LORAWAN_VERSION(const std::string &value);
DEVICECLASS string2deviceclass(const char *value);
