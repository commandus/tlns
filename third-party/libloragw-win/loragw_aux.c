...
uint32_t lora_packet_time_on_air(const uint8_t bw, const uint8_t sf, const uint8_t cr, const uint16_t n_symbol_preamble,
    ...
    if (IS_LORA_DR(sf) == false) {
        ERROR_PRINTF("ERROR: wrong datarate - %s\n", __FUNCTION__);
        return 0;
    }
    ...
    if (IS_LORA_CR(cr) == false) {
        ERROR_PRINTF("ERROR: wrong coding rate - %s\n", __FUNCTION__);
        return 0;
    }
    ...
    switch (bw) {
        ...
        case BW_500KHZ:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: unsupported bandwith 0x%02X (%s)\n", bw, __FUNCTION__);
            return 0;
    }
    ...
    INFO_PRINTF("LoRa packet ToA: %u us (n_symbol:%f, t_symbol_us:%u)\n", toa_us, n_symbol, t_symbol_us);
