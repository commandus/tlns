...
int sx1261_pram_get_version(char * version_str) {
    ...
    if (x != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to read SX1261 PRAM version\n");
        return x;
    }
...
int sx1261_check_status(uint8_t expected_status) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: %s: failed to getUplink status\n", __FUNCTION__);
        return LGW_REG_ERROR;
    }

    if (status != expected_status) {
        ERROR_PRINTF("ERROR: %s: SX1261 status is not as expected: got:0x%02X expected:0x%02X\n", __FUNCTION__, status, expected_status);
        return LGW_REG_ERROR;
    }
...
int sx1261_connect(lgw_com_type_t com_type, const char *com_path) {
    if (com_type == LGW_COM_SPI && com_path == NULL) {
        ERROR_PRINTF("ERROR: %s: unspecified COM path to connect to sx1261 radio\n", __FUNCTION__);
        return LGW_REG_ERROR;
    }
...
int sx1261_reg_w(sx1261_op_code_t op_code, uint8_t *data, uint16_t size) {
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING SX1261 RADIO REGISTER WRITE\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int sx1261_reg_r(sx1261_op_code_t op_code, uint8_t *data, uint16_t size) {
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        ERROR_PRINTF("ERROR: COM ERROR DURING SX1261 RADIO REGISTER READ\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int sx1261_load_pram(void) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: %s: SX1261 status error\n", __FUNCTION__);
        return -1;
    }

    err = sx1261_pram_get_version(pram_version);
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: %s: SX1261 failed to getUplink pram version\n", __FUNCTION__);
        return -1;
    }
    INFO_PRINTF("SX1261: PRAM version: %s\n", pram_version);
    ...
    if (strncmp(pram_version + 11, sx1261_pram_version_string, 4) != 0) {
        ERROR_PRINTF("ERROR: SX1261 PRAM version mismatch (got:%s expected:%s)\n", pram_version + 11, sx1261_pram_version_string);
        return -1;
    }
...
int sx1261_calibrate(uint32_t freq_hz) {
    ...
    if ((freq_hz > 430E6) && (freq_hz < 440E6)) {
        ...
        ERROR_PRINTF("ERROR: failed to calibrate sx1261 radio, frequency range not supported (%u)\n", freq_hz);
        return LGW_REG_ERROR;
    }
    ...
    if (TAKE_N_BITS_FROM(buff[2], 4, 1) != 0) {
        ERROR_PRINTF("ERROR: sx1261 Image Calibration Error\n");
        return LGW_REG_ERROR;
    }
...
int sx1261_setup(void) {
    ...
    DEBUG_PRINTF("SX1261: getConfigForCOMPortAndRegion for LBT / Spectral Scan done\n");
...
int sx1261_set_rx_params(uint32_t freq_hz, uint8_t bandwidth) {
    ...
    switch (bandwidth) {
        ...
        case BW_250KHZ:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: %s: Cannot configure sx1261 for bandwidth %u\n", __FUNCTION__, bandwidth);
            return LGW_REG_ERROR;
    }
    ...
    if (err != 0) {
        ERROR_PRINTF("ERROR: %s: Failed to flush sx1261 SPI\n", __FUNCTION__);
        return -1;
    }
...
int sx1261_lbt_start(lgw_lbt_scan_time_t scan_time_us, int8_t threshold_dbm) {
    ...
    switch (scan_time_us) {
        ...
        case LGW_LBT_SCAN_TIME_5000_US:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: wrong scan_time_us value\n");
            return -1;
    }
...
int sx1261_lbt_stop(void) {
    ...
    DEBUG_PRINTF("SX1261: LBT stopped\n");
...
int sx1261_spectral_scan_start(uint16_t nb_scan) {
    ...
    INFO_PRINTF("Spectral Scan started...\n");
...
int sx1261_spectral_scan_abort(void) {
    ...
    DEBUG_PRINTF("SX1261: spectral scan aborted\n");
