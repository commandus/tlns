...
static int nmea_checksum(const char *nmea_string, int buff_size, char *checksum) {
    ...
    if ((nmea_string == NULL) ||  (checksum == NULL) || (buff_size <= 1)) {
        DEBUG_PRINTF("Invalid parameters for nmea_checksum\n");
        return -1;
    }
    ...
    while (nmea_string[i] != '*') {
        ...
        i += 1;
        if (i >= buff_size) {
            DEBUG_PRINTF("Maximum length reached for nmea_checksum\n");
            return -1;
        }
    }
...
static bool validate_nmea_checksum(const char *serial_buff, int buff_size) {
    ...
    if (checksum_index < 0) {
        DEBUG_PRINTF("ERROR: IMPOSSIBLE TO PARSE NMEA SENTENCE\n");
        return false;
    }

    /* check if there are enough char in the serial buffer to read checksum */
    if (checksum_index >= (buff_size - 2)) {
        DEBUG_PRINTF("ERROR: IMPOSSIBLE TO READ NMEA SENTENCE CHECKSUM\n");
        return false;
    }
...
enum gps_msg lgw_parse_ubx(const char *serial_buff, size_t buff_size, size_t *msg_size) {
    ...
    *msg_size = 0; /* ensure msg_size alway receives a value */
    ...
    if (buff_size < 8) {
        DEBUG_PRINTF("ERROR: TOO SHORT TO BE A VALID UBX MESSAGE\n");
        return IGNORED;
    }

    /* display received serial data and checksum */
    DEBUG_PRINTF("Note: parsing UBX frame> ");
        ...
        if(*msg_size <= buff_size) {
            ...
            if ((ck_a == ck_a_rcv) && (ck_b == ck_b_rcv)) {
                ...
                if ((serial_buff[2] == 0x01) && (serial_buff[3] == 0x20)) {
                    ...
                    DEBUG_PRINTF("NOTE: UBX ACK-NAK received\n");
                    return IGNORED;
                } else if ((serial_buff[2] == 0x05) && (serial_buff[3] == 0x01)) {
                    DEBUG_PRINTF("NOTE: UBX ACK-ACK received\n");
                    return IGNORED;
                ...
                DEBUG_PRINTF("ERROR: UBX message is corrupted, checksum failed\n");
                return INVALID;
            }
        } else { /* message contains less bytes than indicated by header */
            DEBUG_PRINTF("ERROR: UBX message incomplete\n");
            return INCOMPLETE;
        }
...
enum gps_msg lgw_parse_nmea(const char *serial_buff, int buff_size) {
    ...
    if(buff_size > (int)(sizeof(parser_buf) - 1)) {
        DEBUG_PRINTF("Note: input string to big for parsing\n");
        return INVALID;
    }

    /* look for some NMEA sentences in particular */
    if (buff_size < 8) {
        DEBUG_PRINTF("ERROR: TOO SHORT TO BE A VALID NMEA SENTENCE\n");
        return UNKNOWN;
    } else if (!validate_nmea_checksum(serial_buff, buff_size)) {
        DEBUG_PRINTF("Warning: invalid NMEA sentence (bad checksum)\n");
        return INVALID;
        ...
        */
        ...
        if (nb_fields != 13) {
            DEBUG_PRINTF("Warning: invalid RMC sentence (number of fields)\n");
            return IGNORED;
        }
        /* parse GPS status */
        ...
        */
        ...
        nb_fields = str_chop(parser_buf, buff_size, ',', str_index, ARRAY_SIZE(str_index));
        if (nb_fields != 15) {
            DEBUG_PRINTF("Warning: invalid GGA sentence (number of fields)\n");
            return IGNORED;
        }
        ...
        DEBUG_PRINTF("Note: ignored NMEA sentence\n"); /* quite verbose */
        return IGNORED;
    }
}
...
int lgw_gps_get(struct timespec *utc, struct timespec *gps_time, struct coord_s *loc, struct coord_s *err) {
    ...
    if (utc != NULL) {
        if (!gps_time_ok) {
            DEBUG_PRINTF("ERROR: NO VALID TIME TO RETURN\n");
        ...
        x.tm_min = gps_min;
        x.tm_sec = gps_sec;
        y = mktime(&x) - _timezone; /* need to substract timezone bc mktime assumes time vector is local time */
        if (y == (time_t)(-1)) {
            DEBUG_PRINTF("ERROR: FAILED TO CONVERT BROKEN-DOWN TIME\n");
            return LGW_GPS_ERROR;
        }
        utc->tv_sec = y;
    ...
    if (loc != NULL) {
        if (!gps_pos_ok) {
            DEBUG_PRINTF("ERROR: NO VALID POSITION TO RETURN\n");
            return LGW_GPS_ERROR;
        }
    ...
    if (err != NULL) {
        DEBUG_PRINTF("Warning: localization error processing not implemented yet\n");
        err->lat = 0.0;
        err->lon = 0.0;
        err->alt = 0;
    }
...
int lgw_gps_sync(struct tref *ref, uint32_t count_us, struct timespec utc, struct timespec gps_time) {
    ...
    if (utc_diff != 0) { // prevent divide by zero
        ...
        if ((slope > PLUS_10PPM) || (slope < MINUS_10PPM)) {
            DEBUG_PRINTF("Warning: correction range exceeded\n");
            aber_n0 = true;
        } else {
            aber_n0 = false;
        }
    } else {
        DEBUG_PRINTF("Warning: aberrant UTC value for synchronization\n");
        aber_n0 = true;
    }
    ...
    if (aber_n0 == false) {
        ...
        DEBUG_PRINTF("Warning: 3 successive aberrant sync attempts, sync reset\n");
        aber_min2 = aber_min1;
        aber_min1 = aber_n0;
        return LGW_GPS_SUCCESS;
    } else {
...
int lgw_cnt2utc(struct tref ref, uint32_t count_us, struct timespec *utc) {
    ...
    if ((ref.systime == 0) || (ref.xtal_err > PLUS_10PPM) || (ref.xtal_err < MINUS_10PPM)) {
        DEBUG_PRINTF("ERROR: INVALID REFERENCE FOR CNT -> UTC CONVERSION\n");
        return LGW_GPS_ERROR;
    }
...
int lgw_utc2cnt(struct tref ref, struct timespec utc, uint32_t *count_us) {
    ...
    CHECK_NULL(count_us);
    if ((ref.systime == 0) || (ref.xtal_err > PLUS_10PPM) || (ref.xtal_err < MINUS_10PPM)) {
        DEBUG_PRINTF("ERROR: INVALID REFERENCE FOR UTC -> CNT CONVERSION\n");
        return LGW_GPS_ERROR;
    }
...
int lgw_cnt2gps(struct tref ref, uint32_t count_us, struct timespec *gps_time) {
    ...
    CHECK_NULL(gps_time);
    if ((ref.systime == 0) || (ref.xtal_err > PLUS_10PPM) || (ref.xtal_err < MINUS_10PPM)) {
        DEBUG_PRINTF("ERROR: INVALID REFERENCE FOR CNT -> GPS CONVERSION\n");
        return LGW_GPS_ERROR;
    }
...
int lgw_gps2cnt(struct tref ref, struct timespec gps_time, uint32_t *count_us) {
    ...
    CHECK_NULL(count_us);
    if ((ref.systime == 0) || (ref.xtal_err > PLUS_10PPM) || (ref.xtal_err < MINUS_10PPM)) {
        DEBUG_PRINTF("ERROR: INVALID REFERENCE FOR GPS -> CNT CONVERSION\n");
        return LGW_GPS_ERROR;
    }
