...
int sx1302_cal_start(uint8_t version, struct lgw_conf_rxrf_s * rf_chain_cfg, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    if (val != version) {
        ERROR_PRINTF("ERROR: wrong CAL fw version (%d)\n", val);
        return LGW_HAL_ERROR;
    }
    INFO_PRINTF("CAL FW VERSION: %d\n", val);
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        if (rf_chain_cfg[i].enable) {
            ...
            if (cal_status == false) {
                DEBUG_PRINTF("*********************************************\n");
                ERROR_PRINTF("ERROR: Rx image calibration of radio %d failed\n",i);
                DEBUG_PRINTF("*********************************************\n");
                return LGW_HAL_ERROR;
            }
            ...
            INFO_PRINTF("Rx image calibration of radio %d succeeded. Improved image rejection from %2d to %2d dB (Amp:%3d Phi:%3d)\n", i, cal_rx[x_max_idx].rej_init, cal_rx[x_max_idx].rej, cal_rx[x_max_idx].amp, cal_rx[x_max_idx].phi);
        } else {
            rf_rx_image_amp[i] = 0;
            rf_rx_image_phi[i] = 0;
        }
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        if (rf_chain_cfg[i].tx_enable) {
            for (j = 0; j < nb_gains[i]; j++) {
                ...
                if (cal_status == false) {
                    DEBUG_PRINTF("*********************************************\n");
                    ERROR_PRINTF("ERROR: Tx DC offset calibration of radio %d for DAC gain %d and mixer gain %2d failed\n", i, dac_gain[i][j], mix_gain[i][j]);
                    DEBUG_PRINTF("*********************************************\n");
                    return LGW_HAL_ERROR;
                }
                ...
                INFO_PRINTF("Tx DC offset calibration of radio %d for DAC gain %d and mixer gain %2d succeeded. Improved DC rejection by %2d dB (I:%4d Q:%4d)\n", i, dac_gain[i][j], mix_gain[i][j], cal_tx[x_max_idx].rej, cal_tx[x_max_idx].offset_i, cal_tx[x_max_idx].offset_q);
            }
        }
    }
...
int sx125x_cal_rx_image(uint8_t rf_chain, uint32_t freq_hz, bool use_loopback, uint8_t radio_type, struct lgw_sx125x_cal_rx_result_s * res) {
    ...
    switch (radio_type) {
        ...
        case LGW_RADIO_TYPE_SX1257:
            ...
            break;
        default:
            ERROR_PRINTF("ERROR: UNEXPECTED VALUE %d FOR RADIO TYPE\n", radio_type);
            return LGW_HAL_ERROR;
    }
    ...
    if ((rx_pll_locked == 0) || (tx_pll_locked == 0)) {
        ERROR_PRINTF("ERROR: PLL failed to lock\n");
        return LGW_HAL_ERROR;
    }
    ...
    DEBUG_PRINTF("  CAL: waiting for RX calibration to complete...\n");
    sx1302_agc_wait_status((rf_chain == 0) ? 0x11 : 0x22);
    DEBUG_PRINTF("CAL: RX Calibration Done\n");

    INFO_PRINTF("%s, RESULT: rf_chain:%u amp:%d phi:%d\n", __FUNCTION__, rf_chain, res->amp, res->phi);
...
int sx125x_cal_tx_dc_offset(uint8_t rf_chain, uint32_t freq_hz, uint8_t dac_gain, uint8_t mix_gain, uint8_t radio_type, struct lgw_sx125x_cal_tx_result_s * res) {
...
#if DEBUG_CAL == 1
    INFO_PRINTF("TX_SIG values returned by signal analyzer:\n");
    for (i = 0; i < 40; i++) {
        if (i%5 == 0) {
            INFO_PRINTF("\n");
        }
        INFO_PRINTF("%u ", msb[i] * 256 + lsb[i]);
    }
    INFO_PRINTF("\n");
#endif /* DEBUG_CAL */
    ...
    INFO_PRINTF("%s: RESULT: offset_i:%d offset_q:%d rej:%u\n", __FUNCTION__, res->offset_i, res->offset_q, res->rej);

    /* Wait for calibration to be completed */
    DEBUG_PRINTF("waiting for TX calibration to complete...\n");
    sx1302_agc_wait_status((rf_chain == 0) ? 0x33 : 0x44);

#endif /* TX_CALIB_DONE_BY_HAL */

    DEBUG_PRINTF("TX Calibration Done\n");
...
void agc_cal_tx_dc_offset(uint8_t rf_chain, signed char freq, char amp_hal, char phi_hal, char level_reqired, char precision, int8_t * offset_i_res, int8_t * offset_q_res, uint16_t * rej) {
    ...
    for (i = 0; i<sizeof (span); i++) {
        ...
        INFO_PRINTF("abs_corr_min_i16:%d ", abs_corr_min_i16);
        ...
        for (j = 1; j < 5; j++) {
            ...
            abs_corr_i16 = abs_msb * 256 + abs_lsb;
            INFO_PRINTF("abs_corr_i16:%d ", abs_corr_i16);
    ...
    *offset_q_res = (int8_t)offset_q;
    INFO_PRINTF("offset_i:%d offset_q:%d rej:%u\n", offset_i, offset_q, *rej);
}
