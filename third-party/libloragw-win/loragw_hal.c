...
static int remove_pkt(struct lgw_pkt_rx_s * p, uint8_t * nb_pkt, uint8_t pkt_index) {
    ...
    if (pkt_index > ((*nb_pkt) - 1)) {
        ERROR_PRINTF("ERROR: failed to remove packet index %u\n", pkt_index);
        return -1;
    }
...
static int merge_packets(struct lgw_pkt_rx_s * p, uint8_t * nb_pkt) {
    ...
    while (j < cpt) {
        for (k = (j+1); k < cpt; k++) {
            ...
            */
            if (is_same_pkt( &p[j], &p[k])) {
                ...
                if (x != 0) {
                    ERROR_PRINTF("ERROR: failed to remove packet from array (%d)\n", x);
                }
                dup_restart = true;
                break;
            }
        ...
        if (dup_restart == true) {
            ...
            dup_restart = false;
#if 0
            INFO_PRINTF("restarting search for duplicate\n" ); /* Too verbose */
#endif
        } else {
            /* No duplicate found, continue... */
            j += 1;
#if 0
            INFO_PRINTF("no duplicate found\n" ); /* Too verbose */
#endif
        }
    }
...
int lgw_board_setconf(struct lgw_conf_board_s * conf) {
    ...
    if (CONTEXT_STARTED == true) {
        ERROR_PRINTF("ERROR: CONCENTRATOR IS RUNNING, STOP IT BEFORE TOUCHING CONFIGURATION\n");
        return LGW_HAL_ERROR;
    }

    /* Check input parameters */
    if ((conf->com_type != LGW_COM_SPI) && (conf->com_type != LGW_COM_USB)) {
        ERROR_PRINTF("ERROR: WRONG COM TYPE\n");
        return LGW_HAL_ERROR;
    }
...
int lgw_rxrf_setconf(uint8_t rf_chain, struct lgw_conf_rxrf_s * conf) {
    ...
    if ((conf->type != LGW_RADIO_TYPE_SX1255) && (conf->type != LGW_RADIO_TYPE_SX1257) && (conf->type != LGW_RADIO_TYPE_SX1250)) {
        ERROR_PRINTF("ERROR: NOT A VALID RADIO TYPE (%d)\n", conf->type);
        return LGW_HAL_ERROR;
    }

    /* check if the radio central frequency is valid */
    if ((conf->freq_hz < LGW_RF_RX_FREQ_MIN) || (conf->freq_hz > LGW_RF_RX_FREQ_MAX)) {
        ERROR_PRINTF("ERROR: NOT A VALID RADIO CENTER FREQUENCY, PLEASE CHECK IF IT HAS BEEN GIVEN IN HZ (%u)\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
...
int lgw_rxif_setconf(uint8_t if_chain, struct lgw_conf_rxif_s * conf) {
    ...
    if (if_chain >= LGW_IF_CHAIN_NB) {
        ERROR_PRINTF("ERROR: %d NOT A VALID IF_CHAIN NUMBER\n", if_chain);
        return LGW_HAL_ERROR;
    }
    ...
    if (sx1302_get_ifmod_config(if_chain) == IF_UNDEFINED) {
        ERROR_PRINTF("ERROR: IF CHAIN %d NOT CONFIGURABLE\n", if_chain);
    }
    if (conf->rf_chain >= LGW_RF_CHAIN_NB) {
        ERROR_PRINTF("ERROR: INVALID RF_CHAIN TO ASSOCIATE WITH A LORA_STD IF CHAIN\n");
        return LGW_HAL_ERROR;
    }
    ...
    if ((conf->freq_hz + ((bw_hz==-1)?LGW_REF_BW:bw_hz)/2) > ((int32_t)rf_rx_bandwidth/2)) {
        ERROR_PRINTF("ERROR: IF FREQUENCY %d TOO HIGH\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    } else if ((conf->freq_hz - ((bw_hz==-1)?LGW_REF_BW:bw_hz)/2) < -((int32_t)rf_rx_bandwidth/2)) {
        ERROR_PRINTF("ERROR: IF FREQUENCY %d TOO LOW\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
    ...
    switch (sx1302_get_ifmod_config(if_chain)) {
        case IF_LORA_STD:
            ...
            if (!IS_LORA_BW(conf->bandwidth)) {
                ERROR_PRINTF("ERROR: BANDWIDTH NOT SUPPORTED BY LORA_STD IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            if (!IS_LORA_DR(conf->datarate)) {
                ERROR_PRINTF("ERROR: DATARATE NOT SUPPORTED BY LORA_STD IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
        ...
        case IF_LORA_MULTI:
            ...
            if (conf->bandwidth != BW_125KHZ) {
                ERROR_PRINTF("ERROR: BANDWIDTH NOT SUPPORTED BY LORA_MULTI IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            if (!IS_LORA_DR(conf->datarate)) {
                ERROR_PRINTF("ERROR: DATARATE(S) NOT SUPPORTED BY LORA_MULTI IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            /* set internal configuration  */
            CONTEXT_IF_CHAIN[if_chain].enable = conf->enable;
        ...
        case IF_FSK_STD:
            ...
            if(!IS_FSK_BW(conf->bandwidth)) {
                ERROR_PRINTF("ERROR: BANDWIDTH NOT SUPPORTED BY FSK IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            if(!IS_FSK_DR(conf->datarate)) {
                ERROR_PRINTF("ERROR: DATARATE NOT SUPPORTED BY FSK IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            /* set internal configuration  */
            CONTEXT_IF_CHAIN[if_chain].enable = conf->enable;
        ...
        default:
            ERROR_PRINTF("ERROR: IF CHAIN %d TYPE NOT SUPPORTED\n", if_chain);
            return LGW_HAL_ERROR;
    }
...
int lgw_txgain_setconf(uint8_t rf_chain, struct lgw_tx_gain_lut_s * conf) {
    ...
    if ((conf->size < 1) || (conf->size > TX_GAIN_LUT_SIZE_MAX)) {
        ERROR_PRINTF("ERROR: TX gain LUT must have at least one entry and  maximum %d entries\n", TX_GAIN_LUT_SIZE_MAX);
        return LGW_HAL_ERROR;
    }
    ...
    for (i = 0; i < CONTEXT_TX_GAIN_LUT[rf_chain].size; i++) {
        ...
        if (conf->lut[i].dig_gain > 3) {
            ERROR_PRINTF("ERROR: TX gain LUT: SX1302 digital gain must be between 0 and 3\n");
            return LGW_HAL_ERROR;
        }
        if (conf->lut[i].dac_gain > 3) {
            ERROR_PRINTF("ERROR: TX gain LUT: SX1257 DAC gains must not exceed 3\n");
            return LGW_HAL_ERROR;
        }
        if ((conf->lut[i].mix_gain < 5) || (conf->lut[i].mix_gain > 15)) {
            ERROR_PRINTF("ERROR: TX gain LUT: SX1257 mixer gain must be betwen [5..15]\n");
            return LGW_HAL_ERROR;
        }
        if (conf->lut[i].pa_gain > 3) {
            ERROR_PRINTF("ERROR: TX gain LUT: External PA gain must not exceed 3\n");
            return LGW_HAL_ERROR;
        }
        if (conf->lut[i].pwr_idx > 22) {
            ERROR_PRINTF("ERROR: TX gain LUT: SX1250 power index must not exceed 22\n");
            return LGW_HAL_ERROR;
        }
...
int lgw_sx1261_setconf(struct lgw_conf_sx1261_s * conf) {
    ...
    for (i = 0; i < CONTEXT_SX1261.lbt_conf.nb_channel; i++) {
        if (conf->lbt_conf.channels[i].bandwidth != BW_125KHZ && conf->lbt_conf.channels[i].bandwidth != BW_250KHZ) {
            ERROR_PRINTF("ERROR: bandwidth not supported for LBT channel %d\n", i);
            return LGW_HAL_ERROR;
        }
        if (conf->lbt_conf.channels[i].scan_time_us != LGW_LBT_SCAN_TIME_128_US && conf->lbt_conf.channels[i].scan_time_us != LGW_LBT_SCAN_TIME_5000_US) {
            ERROR_PRINTF("ERROR: scan_time_us not supported for LBT channel %d\n", i);
            return LGW_HAL_ERROR;
        }
...
int lgw_start(void) {
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to set all GPIOs to 0\n");
        return LGW_HAL_ERROR;
    }
        ...
        if (CONTEXT_RF_CHAIN[i].enable == true) {
            ...
            switch (CONTEXT_RF_CHAIN[i].type) {
                ...
                case LGW_RADIO_TYPE_SX1257:
                    ...
                    break;
                default:
                    ERROR_PRINTF("ERROR: RADIO TYPE NOT SUPPORTED (RF_CHAIN %d)\n", i);
                    return LGW_HAL_ERROR;
            }
            if (err != LGW_REG_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to getConfigForCOMPortAndRegion radio %d\n", i);
                return LGW_HAL_ERROR;
            }
            ...
            if (err != LGW_REG_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to set mode for radio %d\n", i);
                return LGW_HAL_ERROR;
            }
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to getUplink clock from radio %u\n", CONTEXT_BOARD.clksrc);
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to release control over radios\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to initialize SX1302\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure SX1302 PA/LNA LUT\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure SX1302 radio frontend\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure SX1302 channelizer\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure SX1302 LoRa modem correlators\n");
        return LGW_HAL_ERROR;
    }
    err = sx1302_lora_modem_configure(CONTEXT_RF_CHAIN[0].freq_hz);
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure SX1302 LoRa modems\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (CONTEXT_IF_CHAIN[8].enable == true) {
        ...
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to configure SX1302 LoRa Service modem correlators\n");
            return LGW_HAL_ERROR;
        }
        err = sx1302_lora_service_modem_configure(&(CONTEXT_LORA_SERVICE), CONTEXT_RF_CHAIN[0].freq_hz);
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to configure SX1302 LoRa Service modem\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    if (CONTEXT_IF_CHAIN[9].enable == true) {
        ...
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to configure SX1302 FSK modem\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to configure SX1302 LoRa syncword\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to enable SX1302 modems\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (log_file == NULL) {
        ...
        INFO_PRINTF("%s file opened for debug log\n", CONTEXT_DEBUG.log_file_name);
    ...
    if (CONTEXT_COM_TYPE == LGW_COM_SPI) {
        ...
        for (i = 0; i < (int)(sizeof I2C_PORT_TEMP_SENSOR); i++) {
            ...
            if (err != LGW_I2C_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to open I2C for temperature sensor on port 0x%02X\n", ts_addr);
                return LGW_HAL_ERROR;
            }

            err = stts751_configure(ts_fd, ts_addr);
            if (err != LGW_I2C_SUCCESS) {
                INFO_PRINTF("INFO: no temperature sensor found on port 0x%02X\n", ts_addr);
        ...
        if (i == sizeof I2C_PORT_TEMP_SENSOR) {
            ERROR_PRINTF("ERROR: no temperature sensor found.\n");
            return LGW_HAL_ERROR;
        }
#endif
        ...
        if (CONTEXT_BOARD.full_duplex == true) {
            ...
            if (err != LGW_I2C_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to open I2C for ad5338r\n");
                return LGW_HAL_ERROR;
            }

            err = ad5338r_configure(ad_fd, I2C_PORT_DAC_AD5338R);
            if (err != LGW_I2C_SUCCESS) {
                ERROR_PRINTF("ERROR: failed to configure ad5338r\n");
            ...
            if (err != LGW_I2C_SUCCESS) {
                ERROR_PRINTF("ERROR: AD5338R: failed to set DAC output to 0V\n");
                return LGW_HAL_ERROR;
            }
    ...
    if (CONTEXT_SX1261.enable == true) {
        ...
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to connect to the sx1261 radio (LBT/Spectral Scan)\n");
            return LGW_HAL_ERROR;
        }

        err = sx1261_load_pram();
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to patch sx1261 radio for LBT/Spectral Scan\n");
            return LGW_HAL_ERROR;
        }

        err = sx1261_calibrate(CONTEXT_RF_CHAIN[0].freq_hz);
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to calibrate sx1261 radio\n");
            return LGW_HAL_ERROR;
        }

        err = sx1261_setup();
        if (err != LGW_REG_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to getConfigForCOMPortAndRegion sx1261 radio\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to set CONFIG_DONE GPIO\n");
        return LGW_HAL_ERROR;
    }
...
int lgw_stop(void) {
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        INFO_PRINTF("Aborting TX on chain %u\n", i);
        x = lgw_abort_tx(i);
        if (x != LGW_HAL_SUCCESS) {
            INFO_PRINTF("WARNING: failed to getUplink abort TX on chain %u\n", i);
            err = LGW_HAL_ERROR;
        }
    }
    ...
    x = lgw_disconnect();
    if (x != LGW_HAL_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to disconnect concentrator\n");
        err = LGW_HAL_ERROR;
    }

    if (CONTEXT_COM_TYPE == LGW_COM_SPI) {
        INFO_PRINTF("Closing I2C for temperature sensor\n");
        x = i2c_linuxdev_close(ts_fd);
        if (x != 0) {
            ERROR_PRINTF("ERROR: failed to close I2C temperature sensor device (err=%i)\n", x);
            err = LGW_HAL_ERROR;
        }

        if (CONTEXT_BOARD.full_duplex == true) {
            INFO_PRINTF("Closing I2C for AD5338R\n");
            x = i2c_linuxdev_close(ad_fd);
            if (x != 0) {
                ERROR_PRINTF("ERROR: failed to close I2C AD5338R device (err=%i)\n", x);
                err = LGW_HAL_ERROR;
            }
        }
    }
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    if (res != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to fetch packets from SX1302\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (nb_pkt_fetched > max_pkt) {
        ...
        INFO_PRINTF("WARNING: not enough space allocated, fetched %d packet(s), %d will be left in RX buffer\n", nb_pkt_fetched, nb_pkt_left);
    }

    /* Apply RSSI temperature compensation */
    res = lgw_get_temperature(&current_temperature);
    if (res != LGW_I2C_SUCCESS) {
        ERROR_PRINTF("ERROR: failed to getUplink current temperature\n");
        return LGW_HAL_ERROR;
    }
    ...
    for (nb_pkt_found = 0; nb_pkt_found < ((nb_pkt_fetched <= max_pkt) ? nb_pkt_fetched : max_pkt); nb_pkt_found++) {
        ...
        if (res == LGW_REG_WARNING) {
            INFO_PRINTF("WARNING: parsing error on packet %d, discarding fetched packets\n", nb_pkt_found);
            return LGW_HAL_SUCCESS;
        } else if (res == LGW_REG_ERROR) {
            ERROR_PRINTF("ERROR: fatal parsing error on packet %d, aborting...\n", nb_pkt_found);
            return LGW_HAL_ERROR;
        }
        ...
        INFO_PRINTF("RSSI temperature offset applied: %.3f dB (current temperature %.1f C)\n", rssi_temperature_offset, current_temperature);
    }

    INFO_PRINTF("nb pkt found:%u left:%u\n", nb_pkt_found, nb_pkt_left);
    ...
    if ((nb_pkt_found > 0) && (CONTEXT_FINE_TIMESTAMP.enable == true)) {
        ...
        if (res != 0) {
            INFO_PRINTF("WARNING: failed to remove duplicated packets\n");
        }

        INFO_PRINTF("nb pkt found:%u (after de-duplicating)\n", nb_pkt_found);
    }
...
int lgw_send(struct lgw_pkt_tx_s * pkt_data) {
    ...
    /* check if the concentrator is runningUplink */
    if (CONTEXT_STARTED == false) {
        ERROR_PRINTF("ERROR: CONCENTRATOR IS NOT RUNNING, START IT BEFORE SENDING\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (pkt_data->rf_chain >= LGW_RF_CHAIN_NB) {
        ERROR_PRINTF("ERROR: INVALID RF_CHAIN TO SEND PACKETS\n");
        return LGW_HAL_ERROR;
    }

    /* check input variables */
    if (CONTEXT_RF_CHAIN[pkt_data->rf_chain].tx_enable == false) {
        ERROR_PRINTF("ERROR: SELECTED RF_CHAIN IS DISABLED FOR TX ON SELECTED BOARD\n");
        return LGW_HAL_ERROR;
    }
    if (CONTEXT_RF_CHAIN[pkt_data->rf_chain].enable == false) {
        ERROR_PRINTF("ERROR: SELECTED RF_CHAIN IS DISABLED\n");
        return LGW_HAL_ERROR;
    }
    if (!IS_TX_MODE(pkt_data->tx_mode)) {
        ERROR_PRINTF("ERROR: TX_MODE NOT SUPPORTED\n");
        return LGW_HAL_ERROR;
    }
    if (pkt_data->modulation == MOD_LORA) {
        if (!IS_LORA_BW(pkt_data->bandwidth)) {
            ERROR_PRINTF("ERROR: BANDWIDTH NOT SUPPORTED BY LORA TX\n");
            return LGW_HAL_ERROR;
        }
        if (!IS_LORA_DR(pkt_data->datarate)) {
            ERROR_PRINTF("ERROR: DATARATE NOT SUPPORTED BY LORA TX\n");
            return LGW_HAL_ERROR;
        }
        if (!IS_LORA_CR(pkt_data->coderate)) {
            ERROR_PRINTF("ERROR: CODERATE NOT SUPPORTED BY LORA TX\n");
            return LGW_HAL_ERROR;
        }
        if (pkt_data->size > 255) {
            ERROR_PRINTF("ERROR: PAYLOAD LENGTH TOO BIG FOR LORA TX\n");
            return LGW_HAL_ERROR;
        }
    } else if (pkt_data->modulation == MOD_FSK) {
        if((pkt_data->f_dev < 1) || (pkt_data->f_dev > 200)) {
            ERROR_PRINTF("ERROR: TX FREQUENCY DEVIATION OUT OF ACCEPTABLE RANGE\n");
            return LGW_HAL_ERROR;
        }
        ...
        if (pkt_data->size > 255) {
            ERROR_PRINTF("ERROR: PAYLOAD LENGTH TOO BIG FOR FSK TX\n");
            return LGW_HAL_ERROR;
        }
    } else if (pkt_data->modulation == MOD_CW) {
        /* do nothing */
    } else {
        ERROR_PRINTF("ERROR: INVALID TX MODULATION\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (CONTEXT_BOARD.full_duplex == true) {
        ...
        err = ad5338r_write(ad_fd, I2C_PORT_DAC_AD5338R, volt_val);
        if (err != LGW_I2C_SUCCESS) {
            ERROR_PRINTF("ERROR: failed to set voltage by ad5338r\n");
            return LGW_HAL_ERROR;
        }
    ...
    if (CONTEXT_SX1261.lbt_conf.enable == true) {
        ...
        if (err != 0) {
            ERROR_PRINTF("ERROR: failed to start LBT\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: %s: Failed to send packet\n", __FUNCTION__);

        if (CONTEXT_SX1261.lbt_conf.enable == true) {
            err = lgw_lbt_stop();
            if (err != 0) {
                ERROR_PRINTF("ERROR: %s: Failed to stop LBT\n", __FUNCTION__);
            }
        }
    ...
    if (CONTEXT_SX1261.lbt_conf.enable == true) {
        ...
        if (err != 0) {
            ERROR_PRINTF("ERROR: %s: Failed to getUplink LBT TX status, TX aborted\n", __FUNCTION__);
            err = sx1302_tx_abort(pkt_data->rf_chain);
            if (err != 0) {
                ERROR_PRINTF("ERROR: %s: Failed to abort TX\n", __FUNCTION__);
            }
            err = lgw_lbt_stop();
            if (err != 0) {
                ERROR_PRINTF("ERROR: %s: Failed to stop LBT\n", __FUNCTION__);
        ...
        if (lbt_tx_allowed == true) {
            INFO_PRINTF("LBT: packet is allowed to be transmitted\n");
        } else {
            INFO_PRINTF("LBT: (ERROR) packet is NOT allowed to be transmitted\n");
        }
...
uint32_t lgw_time_on_air(const struct lgw_pkt_tx_s *packet) {
   ...
   DEBUG_PRINTF(" --- %s\n", "IN");
    ...
    if (packet->modulation == MOD_LORA) {
       ...
       INFO_PRINTF("LoRa packet ToA: %u ms\n", toa_ms);
...
int lgw_spectral_scan_start(uint32_t freq_hz, uint16_t nb_scan) {
    ...
    if (CONTEXT_SX1261.enable != true) {
        ERROR_PRINTF("ERROR: sx1261 is not enabled, no spectral scan\n");
        return LGW_HAL_ERROR;
    }

    err = sx1261_set_rx_params(freq_hz, BW_125KHZ);
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: Failed to set RX params for Spectral Scan\n");
        return LGW_HAL_ERROR;
    }

    err = sx1261_spectral_scan_start(nb_scan);
    if (err != LGW_REG_SUCCESS) {
        ERROR_PRINTF("ERROR: start spectral scan failed\n");
        return LGW_HAL_ERROR;
    }
